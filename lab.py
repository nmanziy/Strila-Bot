...
filename = f"dialog-{now.strftime('%Y-%m-%d_%H-%M-%S')}.txt"

# Відкриваємо файл на запис та записуємо діалог
with open(filename, 'w') as file:
    while True:
        user_input = input("Введіть ваше повідомлення: ")
        file.write(f"User: {user_input}\n")
        # Додатковий код для обробки введеного тексту
        # ...
        bot_response = "Відповідь бота"
...
def print_response(text, color='yellow'):
    colors = {
        'black': '\033[30m',
        'red': '\033[31m',
        'green': '\033[32m',
...
# Create new file for dialogue session
